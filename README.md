...
<img src="assets/desktop.png" width="820" alt="חלון האפליקציה: מקלט, סימון הודעות, וערכים חיים">

<sub>הערכים בתמונה מפוענחים על ידי הפרסר האמיתי מזרם GREIS מוקלט שנמצא ב-repo, לא מוקלדים.</sub>

<br>

<img src="assets/iphone.png" width="820" alt="אפליקציית האייפון: Link, Log ו-Recording">
...
חמש הודעות GREIS, ועוד שתי רשומות שמחושבות כאן ולא מתבקשות מהמקלט. כל
אחת תורמת את העמודות שלה לקובץ, ושום דבר אחר באפליקציה לא צריך להשתנות
כשמוסיפים עוד אחת.
...
| **Position in radians** | קו רוחב וקו אורך ברדיאנים. **מחושב כאן** — ראה למטה. |
...
## רדיאנים — עמודה מחושבת

`Position in radians` מוסיפה שתי עמודות, `lat_rad` ו-`lon_rad`, בשתים
עשרה ספרות אחרי הנקודה.

**שתיים ולא שלוש.** לגובה אין צורת רדיאנים ולא צריכה להיות: גובה הוא
אורך, לא זווית. `alt_m` הוא הצורה היחידה שלו, ועמודת `alt_rad` הייתה
המצאה של יחידה שלא קיימת.

יש כאן פרט שכדאי להכיר: `[PG]` שולח את קווי הרוחב והאורך **ברדיאנים**
מלכתחילה, והפרסר ממיר אותם למעלות פעם אחת בקצה — כדי ששני מקומות בקוד
לא יחלקו על היחידה של קו רוחב. העמודה הזאת ממירה בחזרה. המעבר הלוך-חזור
עולה בערך יחידה אחת במקום האחרון של `double`, יחס של 1e-16, שהוא ננומטר
על פני כדור הארץ — מול מקלט שהערכת השגיאה שלו נמדדת במילימטרים.

שתים עשרה ספרות של רדיאן הן 1e-12 רדיאן, בערך מאית המילימטר על הקרקע.
זה עדין יותר מתשע הספרות של המעלות שלידן, כך שעמודת הרדיאנים לעולם אינה
זו שמעגלת.

## ECEF — עמודה מחושבת

קו רוחב, קו אורך וגובה אליפסואידי מציינים בדיוק את אותה נקודה כמו X, Y
ו-Z ממרכז כדור הארץ, והמעבר ביניהם הוא נוסחה סגורה ומדויקת — לא הערכה
ולא קירוב.
